<div>
    ...
    <select id="football_match_id" name="football_match_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
        <option value="">Pilih pertandingan</option>
        @foreach ($matches as $match)
            <option value="{{ $match->id }}" @selected(old('football_match_id', $statistic?->football_match_id) == $match->id)>
...
<div>
    ...
    <select id="player_id" name="player_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
        <option value="">Pilih pemain</option>
        @foreach ($players as $player)
            <option value="{{ $player->id }}" @selected(old('player_id', $statistic?->player_id) == $player->id)>
